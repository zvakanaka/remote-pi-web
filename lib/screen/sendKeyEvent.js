...
const clickcliKey = require('./clickcliKeyMap') 
...
function getValidKey(obj) {
  ...
  else if (process.platform === 'darwin') return clickcliKey(obj.key, obj.ctrl, obj.alt, obj.shift)
...
function getKeyCommand(validKey) {
  if (process.platform === 'linux') return `xdotool key ${validKey}`
    ...
    // clickcli works differently from xdotool in that 'kp' is for non-character keys and 't' is for text, and modifier keys require 'kd' (key down) and 'ku' (up)
    return `clickcli ${validKey}`
  }
  platformNotSupported()
}
