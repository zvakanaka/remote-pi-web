...
const isGnomeWayland = isWayland && isGnome
const isWlroots     = isWayland && !isGnome
...
module.exports = async function setupNativeGetScreen(divider = process.env.DIVIDER || 1) {
  ...
  let library
  if (isGnomeWayland) {
    library = require('./gnome_cap').library
  } else {
    const execName = isWlroots ? 'wayland_cap' : 'screen_cap'
